...
/*
module "container_network_profile" {
  source = "../../Networking/NetworkProfile/"
  network_profile_name = "niaaa_network_profile"
  network_profile_location = "${module.virtual_network.virtual_network_location}"
  network_profile_resource_group_name = "${module.virtual_network.virtual_network_resource_group_name}"
  network_profile_container_network_interface = [{
    "name" = "container-niaaa"
    "ip_configuration" = {
      "name" = "niaaa_ip_configuration"
      "subnet_id" = "${module.virtual_network_subnet.subnet_id}"
    }
  }]
}
...
module "container_group" {
  source = "../../ContainerResources/ContainerGroup/"
  container_group_name = "example-continst"
  container_group_resource_group_name = "${module.virtual_network.virtual_network_resource_group_name}"
  container_group_location = "${module.virtual_network.virtual_network_location}"
  container_group_container = [
    {
    name   = "niaaa-api"
    image  = "developcontainer.azurecr.io/niaaa-api:v2019.1202.1"
    cpu    = "0.5"
    memory = "1.5"
  ...
  {
  name   = "niaaa-ui"
  image  = "developcontainer.azurecr.io/niaaa-ui:v2019.1202.1"
  cpu    = "0.5"
  memory = "1.5"
  ...
  ports = {
    ...
    container_group_image_registry_credential = [
      {
      username = "developcontainer"
      password = "=cFTKRCodqBQp0A4AF1u=cIHK6yXEF/D"
      server = "developcontainer.azurecr.io"
      }
  ]
  container_group_os_type = "Linux"
  container_group_dns_name_label = "aci-label"
  container_group_ip_address_type = "Private"
  container_group_network_profile_id = "${module.container_network_profile.network_profile_id}"
}
*/
